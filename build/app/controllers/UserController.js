...
class UserController {
  async store(req, res) {
    ...
    const userExist = await _Users2.default.findOne({ where: { email: req.body.email } });
    if (userExist) {
      return res.status(400).json({ error: 'User already exists.' });
    }
    const { id, name, email, provider } = await _Users2.default.create(req.body);
    return res.json({
      id,
      name,
      email,
  ...
  async update(req, res) {
    ...
    if (email !== user.email) {
      const userExist = await _Users2.default.findOne({ where: { email } });
      if (userExist) {
        return res.status(400).json({ error: 'User already exists.' });
      }
    }
