"use strict"; function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }Object.defineProperty(exports, "__esModule", {value: true});var _sysaid = require('./sysaid'); var _sysaid2 = _interopRequireDefault(_sysaid);
var _redmine = require('../../config/redmine'); var _redmine2 = _interopRequireDefault(_redmine);
...
const RedmineApi = function(){
    ...
    return {
        getIssues: async () => {
            const url = `${_redmine2.default.server}/issues.json${_redmine2.default.filters}`;            
            //console.log(`Connecting to remote server on: ${url}`);
            var options = {
                url,
                method: 'GET',
                headers: {
                    'X-Redmine-API-Key': `${_redmine2.default.api_key}`
                }
            };            
        ...
        getIssue: async (id_issue = 1) => {
            const url = `${_redmine2.default.server}/issues/${id_issue}.json`;            
            //console.log(`Connecting to remote server on: ${url}`);
            var options = {
                url,
                method: 'GET',
        ...
        getSysAidRequest: async (id_sr) => {
            try{
                const url = `http://localhost:${_auth2.default.serverPort}/sysaid/${id_sr}`;            
                //console.log(`Connecting to remote server on: ${url}`);
                var options = {
                    url,
                    method: 'GET',
        ...
        getSysAidUser: async (id_user) => {
            try{
                const url = `http://localhost:${_auth2.default.serverPort}/user/${id_user}`;            
                //console.log(`Connecting to remote server on: ${url}`);
                var options = {
                    url,
                    method: 'GET',
        ...
        createIssue: async (id_sr) => {
            ...
            try{
                ...
                var options = {
                    uri: `${_redmine2.default.server}/issues.json`,
                    method: 'POST',
                    headers: {
                        'X-Redmine-API-Key': `${_redmine2.default.api_key}`
                    },
                ...
                return new Promise(function (resolve, reject) {
                    _request2.default.call(void 0, options, function (error, res, body) {
                        if (!error && (res.statusCode >= 200 && res.statusCode < 300)) {                            
                            const resp = body;
                            _sysaid2.default.LinkSysAid(id_sr, resp.issue.id).catch(error => {console.log(error)});
                            resolve(resp);
                        } else {
                            console.log(body);
                            reject(error);
...
exports. default = new RedmineApi();
